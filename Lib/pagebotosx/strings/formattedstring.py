...
from pagebotosx.osxcolor import OSXColor, OSXCMYKColor
from pagebotosx.strings.tryinstallfont import _tryInstallFontFromFontName
...
class FormattedString:
                ...
                for featureTag, value in orderedOpenTypeFeatures:
                    ...
                    if coreTextFeatureTag in featureMap:
                        if value and featureTag not in existingOpenTypeFeatures:
                            ...
                            logger.warning("OpenType feature '%s' not available for '%s'", featureTag, self._font)
            ...
            if self._fontVariations:
                ...
                for axis, value in self._fontVariations.items():
                    if axis in existingAxes:
                        ...
                        logger.warning("variation axis '%s' not available for '%s'", axis, self._font)
            fontAttributes = {}
            if coreTextfeatures:
    ...
    def fallbackFont(self, font):
        ...
        if font:
            ...
            if testFont is None:
                raise PageBotOSXError("Fallback font '%s' is not available" % font)
    ...
    def openTypeFeatures(self, *args, **features):
        ...
        if args and features:
            raise PageBotOSXError("Can't combine positional arguments and keyword arguments")
        if args:
            if len(args) != 1:
                raise PageBotOSXError("There can only be one positional argument")
            if args[0] is not None:
                raise PageBotOSXError("First positional argument can only be None")
    ...
    def appendGlyph(self, *glyphNames):
        ...
        if self._font:
            font = AppKit.NSFont.fontWithName_size_(self._font, self._fontSize)
        if font is None:
            logger.warning("font: '%s' is not installed, back to the fallback font: '%s'", self._font, _FALLBACKFONT)
            font = AppKit.NSFont.fontWithName_size_(_FALLBACKFONT, self._fontSize)
